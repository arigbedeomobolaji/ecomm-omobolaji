	...
	return async (dispatch, getState) => {
		...
		try {
			const { data } = await axios.put(
				`${apiBaseUrl}/api/orders/${order._id}/pay`,
				reference,
				{
					headers: {
						authorization: `Bearer ${userInfo.token}`,
...
export const listOrderMine = () => {
	return async (dispatch, getState) => {
		...
		try {
			const { data } = await axios.get(`${apiBaseUrl}/api/orders/mine`, {
				headers: {
					authorization: `Bearer ${userInfo.token}`,
				},
			});
			...
			const message =
				err.reponse && err.response.data.error
					? err.response.data.error
					: err.message;
